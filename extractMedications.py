...
for filename in os.listdir(path):
    ...
    for medications in df[df["textsem"] == "MedicationMention"].groupby("id"):
        if (np.any(medications[1]['subject'] == 'patient')):
            medicationStatement = FHIRHelper.createMedicationStatement(medications[1], reference)
            medicationStatement.update(smart.server)
